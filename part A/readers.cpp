...
void readItems(HashTable<item> &MenItems, HashTable<item> &WomenItems, HashTable<item> &KidsItems)
{
...
bool findcustomers(vector<customer> &customers)
{
...
void writeitems(HashTable<item> &MenItems, HashTable<item> &WomenItems, HashTable<item> &KidsItems)
{
    ofstream file;
    file.open("finalitems.csv");

    for (int i = 0; i < MenItems.getArraySize(); i++)
    {
        item temp = MenItems.FindWithIndex(i);
        if (temp.getID() > 0)
    ...
    for (int i = 0; i < WomenItems.getArraySize(); i++)
    {
        item temp = WomenItems.FindWithIndex(i);
        if (temp.getID() > 0)
            if (temp.getQuantity() > 0)
            {
                file << "women"
    ...
    for (int i = 0; i < KidsItems.getArraySize(); i++)
    {
        item temp = KidsItems.FindWithIndex(i);
        if (temp.getID() > 0)
            if (temp.getQuantity() > 0)
